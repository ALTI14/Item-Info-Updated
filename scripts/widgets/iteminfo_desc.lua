...
local function IsValidContainerItem(item)
    return item ~= nil and item.replica ~= nil and item.replica.container ~= nil
end

function ItemInfoDesc:GetSpoilModifier(inst, owner)
    local modifier = 1.0
    local inventory = owner.replica and owner.replica.inventory
    local body_item = inventory and inventory:GetEquippedItem(EQUIPSLOTS.BODY)
    local is_valid_body_item = IsValidContainerItem(body_item)

    -- Direct checks for owner tags
    ...
    -- Check body item-based modifiers if no owner tag modifier is applied
    if modifier == 1.0 and is_valid_body_item then
        if body_item:HasTag("fridge") and body_item.replica.container:IsHolding(inst, false) then
            modifier = TUNING.PERISH_FRIDGE_MULT
        elseif body_item.prefab == "seedpouch" and body_item.replica.container:IsHolding(inst, false) then
            modifier = TUNING.SEEDPOUCH_PRESERVER_RATE
        elseif body_item:HasTag("foodpreserver") and body_item.replica.container:IsHolding(inst, false) then
...
function ItemInfoDesc:GetColorBasedOnPrefab(prefab_name, item_type)
    ...
    if string.find(prefab_name, "void") then
        return 1.0, 0.8, 0.8, 1.0  -- Reddish white for void-aligned items
    end

    -- Specific checks based on item type and name
    ...
    if item_type == "armor" and string.find(prefab_name, "dreadstone") then
